...
public class GcsUri {
  ...
  private final Supplier<Path> absolutePath;
  ...
  public GcsUri(URI uri) {
    ...
    this.uri = createUri(uri.getAuthority(), uri.getPath().replaceAll(DELIMITER + "+", DELIMITER));
    this.path = memoize(this::calculatePath);
    this.prefix = memoize(this::calculatePrefix);
    this.absolutePath = memoize(this::calculateAbsolutePath);
  }
  ...
  private String calculatePath() {
    if (isNullOrEmpty(uri.getPath()) || uri.getPath().equals(DELIMITER)) {
      return "";
    }
  ...
  public GcsUri resolve(String path) {
    ...
    String resolvedPath = absolutePath.get().resolve(relativePath).toString();
  ...
  public boolean hasSubpath(GcsUri subPath) {
    Path relativePath = absolutePath.get().relativize(subPath.absolutePath.get());
  ...
  public String relativize(GcsUri subPath) {
    Path relativePath = absolutePath.get().relativize(subPath.absolutePath.get());
    checkState(!relativePath.isAbsolute() && !relativePath.startsWith(".."), "Path '%s' is not a subdirectory of '%s'", absolutePath.get(), subPath.absolutePath.get());
